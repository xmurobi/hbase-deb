...
if [ "${UNINSTALL}" -eq "1" ]; then
  ...
  rm -f ${PREFIX}/share/hbase/sbin/hbase-zookeeper
  rm -f /etc/init.d/hbase-master
  rm -f /etc/init.d/hbase-regionserver
  rm -f /etc/init.d/hbase-zookeeper
  ...
  ln -sf ${PREFIX}/share/hbase/sbin/hbase-zookeeper /etc/init.d/hbase-zookeeper
  ...
  echo "export HBASE_MANAGES_ZK=true" >> ${TFILE}
